function J = Analytical_Jacobian(x)
...
for i = 1:M
    for j =1:N             
        ...
        if (j ~= 1 && j~=N)
            vals(3) = vals(3) + Viscous_coeff_x(5);
        else
            vals(3) = 1;
        end
        ...
        % Viscous contribution
        if (i ~= 1 && i~=M)
            vals(12) = vals(12) + Viscous_coeff_y(5);            
        else
            vals(12) = 1;            
        end
        ...
        if (i >2 && i<M-1 && j ~= 1 && j~=N)
            ...
            vals(8) =  vals(8) + Viscous_coeff_x(2);
        ...
        if (i >1 && i<M-1 && j >2 && j~=N)
            ...
            vals(2) =  1;%vals(8) + Viscous_coeff_x(2);
