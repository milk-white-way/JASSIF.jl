function [delta,res,its] = fgmres (myfunc,PRE,precfun,rhs,sol,im,maxits,tolIts) 
%-----------------------------------------------------------------------
% This function is FGMRES with Frechet derivatives evaluation of 
% Jv = ( F(v + dv) - F(v)) / h
% 
%
%-----------------------------------------------------------------------
...
%
  while (its < maxits) 
      vv(1:n,1) = rhs - Frechet_v(myfunc,sol,delta);
      ro = norm(vv(1:n,1),2)  ; 
      res(its+1) = ro;
      if (its  == 0) 
          tol1=tolIts*ro  ;
...
%%                 modified GS  ;
      % Frechet evaluation
      vv(1:n,i1) = Frechet_v(myfunc,sol,z); 
      for j=1:i
